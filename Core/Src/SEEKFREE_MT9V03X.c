...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	if(GPIO_Pin == VSY_Pin)
	{
		...
		__HAL_TIM_ENABLE_DMA(&htim1, TIM_DMA_TRIGGER); 
		 //重置DMA
		HAL_DMA_Start_IT(MT9V032_DMA_CH, MT9V032_DATAPORT, (uint32_t)mt9v03x_image, MT9V03X_IMAGE_SIZE);
	}
}
...
uint8_t mt9v03x_init(void)
{
	...
	// 注册DMA传输完成回调函数
	while(HAL_OK != HAL_DMA_RegisterCallback(MT9V032_DMA_CH, HAL_DMA_XFER_CPLT_CB_ID, DMA_transfer_complete_callback))
    {
        HAL_DMA_Abort_IT(MT9V032_DMA_CH);
    }
