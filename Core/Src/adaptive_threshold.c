...
uint8_t adaptiveThreshold(uint8_t* img_data, int x, int y, int width, int height, int block, uint8_t clip_value) {
  ...
  // block必须为奇数，assert是断言，如果block不是奇数，程序会在这里停止
  assert(block % 2 == 1);
...
void mazeLineTracing(uint8_t* img_data, uint8_t* output_data, int width, int height, int block, uint8_t clip_value) {
    ...
    while (x >= 0) {
      ...
      uint8_t binarized_value1 = adaptiveThreshold(img_data, x - 1, y, width, height, block, clip_value);
      uint8_t binarized_value2 = adaptiveThreshold(img_data, x - 2, y, width, height, block, clip_value);
    ...
    while (x >= 0 && y >= 0 ) {
        // 对当前像素点进行自适应二值化
        uint8_t binarized_value = adaptiveThreshold(img_data, x, y, width, height, block, clip_value);
        ...
        //如果所朝方向是边界，则顺时针旋转90°
        uint8_t binarized_up = adaptiveThreshold(img_data, x + dx[direction], y + dy[direction], width, height, block, clip_value);
        if (binarized_up == 255) {
        ...
        // 判断右侧的像素是否为边界
        uint8_t next_pixel = adaptiveThreshold(img_data, nx, ny, width, height, block, clip_value);
